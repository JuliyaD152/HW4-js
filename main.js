 ...
 * День нужно возвратить в европейской нумерации, т.е. понедельник имеет номер 1, вторник номер 2, …, воскресенье –
 *  номер 7.
 */
let getLocalDay = function(d){
  let date = new Date(d);
  // let localDay = ["7", "1", "2", "3", "4", "5","6"]
  // return localDay[date.getDay()]
  let day = date.getDay();
  if (day === 0) day = 7;
  return day;
 ...
 */
 ...
 let getDateAgo = function (d, days) {
   const date = new Date(d);
...
car.used = 'new';
console.log(car.info()); // chevrolet Lacetti, 2010cc, year 2019, new -- год изменен
car.used = 'used';
console.log(car.info()); // chevrolet Lacetti, 2010cc, year 2019, new -- изменения не выполняются
...
/*
 * #7
 * Напишите функцию testPerformance(iterations, func) для тестирования производительности любых, 
 * переданных ей в качестве параметра функций.
...
// данная функция необходима для корректного тестирования кодa
function test2() {
